...
function QiniuJsSDK() {
    ...
    this.uploader = function (op) {
        ...
        var getFileKey = function (up, file, func) {
            ...
            //return key;
            // mod by xys
            var filetype = key.replace(/.+\./, "");


            var now = new Date().Format("yyMMddhhmmss");
            return filename_type +'/'+  now +   '.' + filetype;
        };
        ...
        uploader.bind('FilesAdded', function (up, files) {
            ...
            //    if (!Qiniu.isImage(files[i].name) &&  up.getOption('isimg' )) {
            //        alert("请选择图片格式的文件");
            //        up.splice(0, 100);
            //        return;
            //    }
            //}
            var displayId = up.getOption('displayId');
              filename_type = up.getOption('filename_type');
            //var maxnum = $('#' + displayId).attr('name') || 8;
            //var currentvalue = $('#' + displayId).find("img").length || 0;
            //if (files.length + currentvalue > maxnum) {
            //    alert("图片数量超出" + maxnum + "张");
            //    up.splice(0, 100);  //图片数未知 所以取了尽量大
            //    return;
            //}
